    ...
    >
      ...
      <motion.div variants={fadeInUp}>
        <Tabs
          ...
          className="w-full max-w-screen-2xl mx-auto my-32 rounded-lg shadow-xl"
        >
          <TabsList className="w-full border-b border-gray-800 bg-transparent">
            {["Description", "details", "reviews", "shipping"].map((tab) => (
              <TabsTrigger
                key={tab}
                value={tab}
                className="flex-1 py-4 text-sm font-semibold uppercase tracking-wide text-gray-400 hover:text-white border-b-2 border-transparent data-[state=active]:border-yellow-500 data-[state=active]:text-white transition-colors"
              >
                {tab === "shipping" ? "Shipping & Returns" : tab}
              </TabsTrigger>
            ))}
          </TabsList>
          <TabsContent value="Description" className="mt-6 p-6 rounded-b-lg">
